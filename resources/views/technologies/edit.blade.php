...
<x-app-layout title="Edit Technology">
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Edit Technology') }}: <span class="text-indigo-600 dark:text-indigo-400">{{ $technology->name }}</span>
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('technologies.update', $technology) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            @if ($technology->icon)
                                <div class="mt-2 mb-3 flex items-center gap-3">
                                    <img id="icon-preview"
                                         src="{{ Storage::url($technology->icon) }}"
                                         alt="{{ $technology->name }}"
